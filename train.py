...
def train_model(model, train_loader: DataLoader, val_loader: Optional[DataLoader], optimizer, criterion, scheduler,
    ...
    for epoch in range(epochs):
        ...
        if loss_to_check < best_loss:
            ...
            model.save(f"{base_path}model_checkpoint.bin", f"{base_path}model_config.json")
    ...
    for i, (inputs, targets, masks) in enumerate(train_loader):
        ...
        if step % accumulation_steps == 0 or step == len(train_loader):
            ...
            if torch.isfinite(grad_norm):
                ...
                print(f"Gradient norm is {grad_norm}. Skipping this batch.")
...
def base_model_train(learning_rate, training_sequence_length, batch_size, max_epochs,
    ...
    if os.path.exists(f"{base_path}model_checkpoint.bin"):
        model.load(f"{base_path}model_checkpoint.bin", config_path)
...
if __name__ == "__main__":
    ...
    while not trained and count < 3:
        # trained = base_model_train(0.005, 512, 64, 400, patience=3, training_folder=TRAIN_FOLDER,
        trained = base_model_train(0.005, 32, 512, 400, patience=3, training_folder=TRAIN_FOLDER,
